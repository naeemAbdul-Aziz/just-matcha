...
const products = [
  {
    ...
    description: "Ceremonial grade, oat milk, agave.",
    price: "$8.00",
  ...
  {
    id: 2,
    name: "Classic Hot Whisk",
    description: "Traditional preparation, hot water.",
    price: "$6.50",
  ...
  },
  {
    ...
    description: "Sparkling water, yuzu, mint.",
    price: "$9.00",
    ...
    <section id="menu" className="py-24 bg-white relative">
      <div className="max-w-7xl mx-auto px-6 lg:px-12">
        <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-6">
          <div>
            <h2 className="font-serif text-4xl lg:text-5xl text-text-dark mb-4">Curated Sips</h2>
            <p className="text-text-light max-w-sm">From the classic ceremonial whisk to adventurous fusions. Taste the difference.</p>
          </div>
          <Link to="/customize" className="text-primary-dark hover:text-primary font-medium flex items-center gap-1 transition-colors">
            View Full Menu <span className="material-icons text-sm">arrow_forward</span>
          </Link>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {products.map((product) => (
            <div key={product.id} className="group cursor-pointer">
              <div className="relative bg-background-light rounded-3xl p-8 mb-6 overflow-hidden transition-all duration-300 group-hover:bg-soft-green/30">
                {product.tag && (
              ...
              <div className="flex justify-between items-start">
                <div>
                  <h3 className="font-serif text-xl text-text-dark mb-1 group-hover:text-primary-dark transition-colors">{product.name}</h3>
                  <p className="text-sm text-text-light">{product.description}</p>
                </div>
                <span className="font-medium text-lg text-text-dark">{product.price}</span>
              </div>
            </div>
          ))}
        </div>
