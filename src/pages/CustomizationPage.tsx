...
export const CustomizationPage: React.FC = () => {
  ...
  return (
    <motion.div variants={pageVariants} initial="initial" animate="animate" exit="exit" className={`min-h-[100dvh] w-full transition-colors duration-1000 ease-in-out ${bgColor} relative overflow-hidden flex flex-col font-display`}>
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center overflow-hidden pointer-events-none z-0 opacity-40 mix-blend-overlay">
        <h1 className="text-[25vw] md:text-[18vw] leading-[0.8] font-black text-black/20 dark:text-white/20 whitespace-nowrap select-none text-center uppercase tracking-tighter transform -rotate-12 mt-20">
          ICED<br/>MATCHA<br/>LATTE
        </h1>
      </div>

      {/* Hero Photorealistic Cup */}
      <div className="absolute top-24 md:top-12 left-1/2 -translate-x-1/2 w-full max-w-[280px] md:max-w-md z-10 pointer-events-none flex justify-center">
        <img 
          ...
          className="w-full h-auto object-contain mix-blend-multiply dark:mix-blend-normal opacity-90 transition-transform duration-1000 scale-110 md:scale-100"
          style={{ WebkitMaskImage: 'linear-gradient(to bottom, black 80%, transparent 100%)' }}
        />
      </div>

      {/* Interactive Bottom Drawer (Stacked Cards) */}
      <div className="relative z-30 mt-auto w-full max-w-[600px] mx-auto px-4 pb-6 h-[55dvh] min-h-[450px]">
         <CustomizerControls
